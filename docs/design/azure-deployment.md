...
- Optional Container Apps built-in Entra auth is deployed whenever its feature flag is enabled, so invalid identity-provider inputs cannot silently omit the auth resource and expose the app. It redirects anonymous application traffic to sign-in and excludes only `/health/live` and `/health/ready` so platform probes remain anonymous. Its Entra registration uses `/.auth/login/aad/callback`. Ephemeral PR environments instead keep built-in auth disabled and install the application's isolated per-PR OIDC registration with `/auth/callback` from a fresh trusted job before opening ingress.
...
- The original Container Apps environment and app remain unchanged for rollback. The `-private` app retains the application UAMI and its exact registry, artifact-container, and Application Insights RBAC scopes. Exactly one app carries `azd-service-name: web`: `ca-fc-${resourceToken}-pvt`; the original rollback app deliberately has no azd service tag. With `azure.yaml` service `web` hosted by `containerapp`, `azd` discovers and deploys to that tagged private app. Its environment has `internal: false` and `publicNetworkAccess: 'Enabled'`, and the app has `ingressExternal: true`, so it remains externally reachable over its Container Apps HTTPS FQDN. This template creates no Azure Front Door, custom-domain, or public-DNS cutover; selecting a custom domain or moving external traffic is a separate Product Owner decision. The old app remains a manual rollback target.
...
<!-- native-bicep-fallback: infra/foundry.bicep/contentPolicy | Microsoft.CognitiveServices/accounts/raiPolicies | The selected Cognitive Services account AVM does not expose custom RAI policy creation or deployment policy binding. -->
<!-- native-bicep-fallback: infra/foundry.bicep/foundryModelDeployment | Microsoft.CognitiveServices/accounts/deployments | The selected Cognitive Services account AVM does not expose custom RAI policy creation or deployment policy binding. -->
...
<!-- native-bicep-fallback: infra/web.bicep/containerAppAuthConfig | Microsoft.App/containerApps/authConfigs | The selected Container App AVM does not expose authConfig identity-provider wiring, so Entra auth is configured explicitly on the deployed app. -->
<!-- native-bicep-fallback: infra/web.bicep/privateContainerAppAuthConfig | Microsoft.App/containerApps/authConfigs | The selected Container App AVM does not expose authConfig identity-provider wiring, so Entra auth is configured explicitly on the deployed app. -->
