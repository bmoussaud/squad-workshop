...
- `azure.yaml` runs `infra/scripts/retire_legacy_storage_blob_role.py` as an `azd` **postprovision** migration. Bicep/ARM incremental deployments do not delete an omitted resource, and the old Storage AVM assignment has a different account-scope GUID from the new container-scope assignment. After Bicep successfully creates the direct `artifacts`-container assignment, the migration queries only direct `Storage Blob Data Contributor` assignments for the application principal at the account scope and deletes the single legacy assignment ID it finds. It never targets the deployer or any other principal.
- The migration fails closed if Azure CLI is unavailable or cannot start, the direct container assignment is absent or ambiguous, more than one account-scoped assignment matches, or the account-scoped assignment remains after deletion. `azd` hooks are expected to have Azure CLI available; treating an absent CLI as a successful no-op could silently leave account-wide Blob access in place. Any failure before deletion leaves the legacy account assignment intact; any delete or verification failure makes `azd provision` fail and does not claim cutover. Because the container assignment is verified before deletion, these failures do not create a Blob-access gap. Successful cutover validation must show exactly one direct role assignment on `.../blobServices/default/containers/artifacts`, none on the storage-account scope, and successful private-app Blob read/write. The original app/environment remains available for traffic rollback, but rollback must not redeploy the pre-migration Storage AVM role configuration because that would reintroduce account-wide Blob access. Any emergency regrant is a separate, approved break-glass action.
