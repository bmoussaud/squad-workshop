...
📌 Team update (2026-07-29T11:11:09+02:00): Issue #61 design is now CLOSED. Implementation must use per-PR Entra app registrations, no FANTASY_CARD_AUTH_MODE and no auth-bypass surface anywhere, session signing from Container App secrets with a current/previous two-key ring (not Key Vault), artifact responses with `Cache-Control: private, no-store`, and local dev via a dev Entra app registration at `http://localhost:8000/auth/callback`. Trinity's global-idempotency finding (`application.py:20-25`, `web.py:375-379`, `adapters.py:535-543`) is confirmed as a first-class cross-user data-leak case that must be fixed in the same change set. — decided by Morpheus, Tank, and Switch; recorded by Scribe
