...
- 2026-07-28T20:03:24+02:00 #64 PR environment cap: Diagnosed PR #64's app-tier block as tag-only concurrency accounting counting closed/closing PR resource groups while only PR #64 was currently open. Added open-PR-aware cap accounting via `infra/scripts/pr_env_active_count.py`, updated `pr-environment.yml` to warn on orphaned groups instead of charging them to live cap, and made the janitor also run after teardown workflow completion so trusted default-branch code promptly reaps closed-PR app-tier leaks. Validation: targeted unittest files, compileall, workflow YAML parse, and `git diff --check` passed.
