...
📌 Team update (2026-07-29T11:11:09+02:00): Issue #61 platform position vindicated: Container App secrets with a current/previous two-key verification ring are adequate for the MVP session/OIDC secret story. Key Vault remains a future hardening option, but the repo currently has no Key Vault, so adding one would create new resource, private endpoint, RBAC, and managed-identity secret-read cost disproportionate to this internal-tool MVP. — recorded by Scribe
