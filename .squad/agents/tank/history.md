...
- 2026-07-29 #49: Hardened immutable custom RAI policy `fantasy-cards-content-policy-v1`, inheriting `Microsoft.DefaultV2` and strengthening all four harm categories to Low for prompts and completions. The model deployment explicitly depends on and references the policy. The merged pre-provider application gate keeps `content_policy_rejected` distinct from provider `safety_rejected` and excludes prompts from telemetry. Live deployment is unverified because the recorded Foundry account was absent from the active subscription; do not provision a billable replacement without approval.
